        ...
        <main>
            <div class="buscador">
                <form action="" method="post" style="display: flex; width: 100%;">
                    <input type="search" name="buscar" placeholder="Buscar trajes...">
            ...
            <div id="contenedor-productos" class="contenedor-productos">
                ...
                // Verificar si la lista de productos no es null y tiene productos
                if ($listaProductos != null && count($listaProductos) > 0) {
                    ...
                    // Filtrar productos por categoría "Trajes"
                    $productosTrajes = array_filter($productosFiltrados, function ($producto) {
                        return $producto->getCategoriaProducto() === 'Trajes';
                    });

                    // Filtrar productos por cantidad disponible mayor a 0
                    $productosTrajes = array_filter($productosTrajes, function ($producto) {
                        return $producto->getCantidad() > 0;
                    });

                    // Mostrar productos o mensaje si no hay resultados
                    if (count($productosTrajes) > 0) {
                        foreach ($productosTrajes as $producto) {
                            ...
                            if ($producto->getModalidadProducto() === 'Compra') {
                                ...
                                // Botón "Reservar" para productos de alquiler
                        ...
                        echo "<p>No hay productos disponibles en la categoría de trajes.</p>";
                    }
                } else {
                    echo "<p>No hay productos disponibles en la categoría de trajes.</p>";
                }
